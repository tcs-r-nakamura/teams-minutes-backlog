...
```text
1つのクラスに、関係の薄い責任を詰め込みすぎない
```

ISPは、主に利用する側から見たインターフェースの大きさに注目します。

```text
利用しない機能まで、インターフェースに含めない
```

### 16.2 LSP：リスコフの置換原則

インターフェースが大きすぎると、一部の実装クラスがすべての機能を正しく提供できず、`NotSupportedException`を投げる可能性があります。

- **ISPの視点**：利用する側に不要な機能が見えている
- **LSPの視点**：実装する側が契約どおりに振る舞えない

今回の問題は、両方の視点から確認できます。

### 16.3 DIP：依存性逆転の原則

DIPでは、具体的なクラスではなく抽象に依存します。

```csharp
private readonly IDocumentReader _documentReader;
```

ただし、抽象であれば何でもよいわけではありません。

大きすぎる抽象ではなく、利用する側に合ったインターフェースに依存することで、DIPとISPの両方を活かせます。
